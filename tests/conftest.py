...
@pytest.fixture
def post_in_unpublished_category(
    author,
    location,
...
@pytest.fixture
def unpub_cat_url(unpublished_category) -> str:
    return reverse('blog:category_posts', args=(unpublished_category.slug,))


@pytest.fixture
def unpublished_post_url(unpublished_post) -> str:
    return reverse('blog:post_detail', args=(unpublished_post.id,))


@pytest.fixture
def unpub_cat_post_url(post_in_unpublished_category) -> str:
    return reverse(
        'blog:post_detail',
        args=(post_in_unpublished_category.id,),
    )
